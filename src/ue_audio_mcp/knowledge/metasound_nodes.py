...
from ue_audio_mcp.knowledge.node_schema import MSPin, MSNode


# ===================================================================
# Helper type aliases (for readability only -- not enforced at runtime)
# ===================================================================
Pin = MSPin         # {"name": str, "type": str}
InputPin = MSPin    # {"name": str, "type": str, "required": bool, "default": ...}
NodeDef = MSNode    # full node definition


def _in(name: str, type_: str, *, required: bool = True, default: object = None) -> MSPin:
    """Build an input pin descriptor."""
    pin: dict = {"name": name, "type": type_, "required": required}
    if default is not None:
        pin["default"] = default
    return pin  # type: ignore[return-value]


def _out(name: str, type_: str) -> MSPin:
    """Build an output pin descriptor."""
    return {"name": name, "type": type_}  # type: ignore[return-value]
...
_register(_node(
    ...
    [     _in("Base Frequency", "Float", default=440),
     _in("HarmonicMultipliers", "Float[]", required=False),
     _in("Amplitudes", "Float[]", required=False),
     _in("Phases", "Float[]", required=False),
     _in("Pan Amounts", "Float[]", required=False)],
    [     _out("Out Left Audio", "Audio"),
     _out("Out Right Audio", "Audio")],
...
_register(_node(
    ...
    [     _in("Enabled", "Bool", default=True),
     _in("Bi Polar", "Bool", required=False, default=True),
     _in("Frequency", "Float", default=440),
     _in("Modulation", "Audio", required=False),
     _in("Sync", "Trigger", required=False),
     _in("Phase Offset", "Float", required=False, default=0),
     _in("Glide", "Float", required=False, default=0),
     _in("Type", "Enum")],
    [     _out("Audio", "Audio")],
...
_register(_node(
    ...
    [     _in("Enabled", "Bool", default=True),
     ...
     _in("Voices", "Int32", required=False, default=3),
     _in("Frequency", "Float", default=440),
     _in("Modulation", "Audio", required=False),
     _in("Detune", "Float", required=False, default=-0.25),
     _in("Entropy", "Float", required=False, default=0),
     _in("Blend", "Float", required=False, default=0),
     _in("Glide", "Float", required=False, default=0),
     _in("Pulse Width", "Float", required=False, default=0.5),
...
_register(_node(
    ...
    [     _in("Play", "Trigger", required=False),
     ...
     _in("WaveTable", "WaveTable", required=False),
     _in("Sync", "Trigger", required=False),
     _in("Freq", "Float", default=440),
     _in("PhaseMod", "Audio")],
    [     _out("Out", "Audio")],
...
_register(_node(
    ...
    [     _in("Play", "Trigger", required=False),
     _in("Stop", "Trigger", required=False),
     _in("Sync", "Trigger", required=False),
     _in("WaveTableBank", "WaveTableBankAsset"),
     _in("Index", "Int32"),
     _in("PitchShift", "Float", default=0),
...
_register(_node(
    ...
    [     _in("Rate", "Float", default=5),
     _in("Seed", "Int32", required=False, default=-1),
     _in("Reset Seed", "Trigger", required=False),
     _in("Sync", "Trigger", required=False),
     _in("Interpolation", "Enum", required=False, default=2),
     _in("Rate Jitter", "Float", required=False, default=0),
     _in("Step Limit", "Float", required=False, default=1),
     _in("Min Value", "Float", required=False, default=0),
     _in("Max Value", "Float", required=False, default=1)],
    [     _out("Out", "Float"),
...
_register(_node(
    ...
    [     _in("X", "Audio"),
     _in("Layers", "Int32", required=False, default=1),
     _in("Seed", "Float", required=False, default=-1),
     _in("Min Value", "Float", required=False, default=-1),
     _in("Max Value", "Float", required=False, default=1)],
    [     _out("Output", "Audio"),
     _out("Normalized", "Audio")],
...
_register(_node(
    ...
    [     _in("Trigger Attack", "Trigger", required=False),
     ...
     _in("Attack Time", "Time", required=False, default=0.009999999776482582),
     _in("Decay Time", "Time", required=False, default=0.20000000298023224),
     _in("Sustain Level", "Float", required=False, default=0.5),
     _in("Release Time", "Time", required=False, default=1),
     _in("Attack Curve", "Float", required=False, default=1),
     _in("Decay Curve", "Float", required=False, default=1),
     _in("Release Curve", "Float", required=False, default=1),
     _in("Hard Reset", "Bool", required=False, default=False)],
...
_register(_node(
    ...
    [     _in("Trigger Attack", "Trigger"),
     ...
     _in("Attack Time", "Time", default=0.009999999776482582),
     _in("Decay Time", "Time", default=0.20000000298023224),
     _in("Sustain Level", "Float", default=0.5),
     _in("Release Time", "Time", default=1),
     _in("Attack Curve", "Float", default=1),
     _in("Decay Curve", "Float", default=1),
     _in("Release Curve", "Float", default=1),
     _in("Hard Reset", "Bool", default=False)],
    [     _out("On Attack Triggered", "Trigger"),
     _out("On Decay Triggered", "Trigger"),
     _out("On Sustain Triggered", "Trigger"),
     _out("On Release Triggered", "Trigger"),
...
_register(_node(
    ...
    [     _in("WaveTable", "WaveTable"),
     _in("Play", "Trigger"),
     _in("Stop", "Trigger", required=False),
     _in("Pause", "Trigger", required=False),
     _in("Duration", "Time", default=1),
     _in("Mode", "Enum", default=0),
     _in("Interpolation", "Enum", required=False, default=1)],
    [     _out("OnFinished", "Trigger"),
     _out("Out", "Float")],
...
_register(_node(
    ...
    [     _in("Enable", "Bool", default=True),
     _in("In", "Audio"),
     _in("Attack Time", "Time", default=0.009999999776482582),
     _in("Release Time", "Time", default=0.10000000149011612),
     _in("Peak Mode", "Enum", required=False, default=0)],
    [     _out("Envelope", "Float"),
...
_register(_node(
    ...
    [     _in("Audio", "Audio"),
     ...
     _in("Frequency", "Float", default=1000),
     _in("Q", "Float", default=1),
     _in("Threshold dB", "Float", default=-12),
     _in("Ratio", "Float", default=4),
     _in("Knee", "Float", default=12),
     _in("Range", "Float", default=-60),
     _in("Gain", "Float", default=0),
     _in("AttackTime", "Time", default=0.009999999776482582),
     _in("ReleaseTime", "Time", default=0.10000000149011612),
     _in("EnvelopeMode", "Enum", default=1),
     _in("AnalogMode", "Bool", default=True)],
    [     _out("Audio", "Audio")],
...
_register(_node(
    ...
    [     _in("In", "Audio"),
     _in("Cutoff Frequency", "Float", default=20000),
     _in("Resonance", "Float", default=1)],
    [     _out("Out", "Audio")],
...
_register(_node(
    ...
    [     _in("In", "Audio"),
     _in("Filter Order", "Enum"),
     _in("Phase Compensate", "Bool", default=True),
     _in("Crossover 0", "Float", default=500),
     _in("Crossover 1", "Float", default=1000),
     _in("Crossover 2", "Float", default=1500),
     _in("Crossover 3", "Float", default=2000)],
    [     _out("Band 0 Out", "Audio"),
     _out("Band 1 Out", "Audio"),
     _out("Band 2 Out", "Audio"),
     _out("Band 3 Out", "Audio"),
     _out("Band 4 Out", "Audio")],
...
_register(_node(
    ...
    [     _in("In", "Audio"),
     _in("Delay Time", "Audio", required=False, default=1),
     _in("Dry Level", "Float", default=0),
     _in("Wet Level", "Float", default=1),
     _in("Feedback", "Float", required=False, default=0),
     _in("Max Delay Time", "Time", required=False, default=5),
...
_register(_node(
    ...
    [     _in("In Left", "Audio"),
     _in("In Right", "Audio"),
     _in("Delay Mode", "Enum", required=False, default=2),
     _in("Delay Time", "Time", required=False, default=1),
     _in("Delay Ratio", "Float", required=False, default=0.10000000149011612),
     _in("Dry Level", "Float", default=0),
     _in("Wet Level", "Float", default=1),
     _in("Feedback", "Float", required=False, default=0),
     _in("Max Delay Time", "Time", default=2.5),
     _in("Reset", "Trigger", required=False)],
    [     _out("Out Left", "Audio"),
     _out("Out Right", "Audio")],
...
_register(_node(
    ...
    [     _in("In", "Audio"),
     _in("Pitch Shift", "Float", default=0),
     _in("Delay Length", "Float", default=30)],
    [     _out("Out", "Audio")],
...
_register(_node(
    ...
    [     _in("Input Audio", "Audio"),
     _in("Depth", "Int32", required=False, default=4),
     _in("Feedback", "Float", required=False, default=0.7070000171661377)],
...
_register(_node(
    ...
    [     _in("Bypass", "Bool", default=False),
     _in("In Left", "Audio"),
     _in("In Right", "Audio"),
     _in("Dry Level", "Float", default=1),
     _in("Wet Level", "Float", default=1),
     _in("Delay (ms)", "Float", default=0),
     _in("Gain (dB)", "Float", default=0),
     _in("Bandwidth", "Float", default=0.5),
     _in("Diffusion", "Float", default=0.5),
     _in("Dampening", "Float", default=0.5),
...
_register(_node(
    ...
    [     _in("In Audio", "Audio"),
     ...
     _in("Grain Delay", "Float", default=128),
     _in("Grain Delay Range", "Float", required=False, default=0),
     _in("Grain Duration", "Float", default=128),
     _in("Grain Duration Range", "Float", required=False, default=0),
     _in("Pitch Shift", "Float", default=0),
     _in("Pitch Shift Range", "Float", required=False, default=0),
     _in("Grain Envelope", "Enum", required=False, default=0),
     _in("Max Grain Count", "Int32", required=False, default=16),
     _in("Feedback Amount", "Float", default=0),
     _in("Max Delay Time", "Time", default=2)],
...
_register(_node(
    ...
    [     _in("Bypass", "Bool", default=False),
     _in("Audio", "Audio"),
     _in("Ratio", "Float", default=1.5),
     _in("Threshold dB", "Float", default=-6),
     _in("Attack Time", "Time", default=0.009999999776482582),
     _in("Release Time", "Time", default=0.10000000149011612),
     _in("Lookahead Time", "Time", required=False, default=0.009999999776482582),
     _in("Knee", "Float", required=False, default=10),
     _in("Sidechain", "Audio", required=False),
     _in("Envelope Mode", "Enum", required=False, default=0),
     _in("Analog Mode", "Bool", required=False, default=True),
     _in("Upwards Mode", "Bool", required=False, default=False),
     _in("Wet/Dry", "Float", required=False, default=1)],
    [     _out("Audio", "Audio"),
...
_register(_node(
    ...
    [     _in("Audio", "Audio"),
     _in("Input Gain dB", "Float", required=False, default=0),
     _in("Threshold dB", "Float", default=0),
     _in("Release Time", "Time", default=0.10000000149011612),
     _in("Knee", "Enum", required=False, default=0)],
    [     _out("Audio", "Audio")],
...
_register(_node(
    "Trigger Accumulate (3)", "Triggers",
    "3-input trigger accumulator. Fires output when all 3 inputs have triggered.",
    [_in("In 0", "Trigger"),
     _in("In 1", "Trigger"),
     _in("In 2", "Trigger"),
     _in("Auto Reset", "Bool")],
    ...
    ["trigger", "count", "accumulate", "threshold", "gate"],
    complexity=2,
    class_name="TriggerAccumulator::Trigger Accumulate (3)::None",
))

_register(_node(
    "Trigger Accumulate (4)", "Triggers",
    "4-input trigger accumulator. Fires output when all 4 inputs have triggered.",
    [_in("In 0", "Trigger"),
     _in("In 1", "Trigger"),
     _in("In 2", "Trigger"),
     _in("In 3", "Trigger"),
    ...
    ["trigger", "count", "accumulate", "threshold", "gate"],
    complexity=2,
    class_name="TriggerAccumulator::Trigger Accumulate (4)::None",
))

_register(_node(
    "Trigger Accumulate (5)", "Triggers",
    "5-input trigger accumulator. Fires output when all 5 inputs have triggered.",
    [_in("In 0", "Trigger"),
     _in("In 1", "Trigger"),
     _in("In 2", "Trigger"),
     _in("In 3", "Trigger"),
    ...
    ["trigger", "count", "accumulate", "threshold", "gate"],
    complexity=2,
    class_name="TriggerAccumulator::Trigger Accumulate (5)::None",
))

_register(_node(
    "Trigger Any (3)", "Triggers",
    "Fires output when any of its 3 input triggers fire.",
    [_in("In 0", "Trigger"),
     _in("In 1", "Trigger"),
     _in("In 2", "Trigger")],
    [_out("Out", "Trigger")],
    ["trigger", "any", "or", "combine", "merge"],
    complexity=1,
    class_name="TriggerAny::Trigger Any (3)::None",
))

_register(_node(
    "Trigger Any (4)", "Triggers",
    "Fires output when any of its 4 input triggers fire.",
    [_in("In 0", "Trigger"),
     _in("In 1", "Trigger"),
     _in("In 2", "Trigger"),
     _in("In 3", "Trigger")],
    ...
    ["trigger", "any", "or", "combine", "merge"],
    complexity=1,
    class_name="TriggerAny::Trigger Any (4)::None",
))
...
_register(_node(
    ...
    [     _in("In", "Trigger"),
     _in("Reset", "Trigger", required=False),
     _in("Loop", "Bool", required=False, default=True)],
    [     _out("Out 0", "Trigger"),
     _out("Out 1", "Trigger")],
...
_register(_node(
    "Trigger Sequence (3)", "Triggers",
    "3-output trigger sequencer. Fires outputs in round-robin order (0, 1, 2).",
    [_in("In", "Trigger"),
     _in("Reset", "Trigger", required=False),
     _in("Loop", "Bool", required=False, default=True)],
    [_out("Out 0", "Trigger"),
    ...
    ["trigger", "sequence", "round-robin", "order", "cycle", "step", "rotate"],
    complexity=2,
    class_name="UE::Trigger Sequence (3)::None",
))
...
_register(_node(
    ...
    [     _in("Next", "Trigger", required=False),
     ...
     _in("Enable Shared State", "Bool", default=False),
     _in("Shared State Behavior", "Enum", default=1)],
...
_register(_node(
    ...
    [     _in("Next", "Trigger"),
     _in("Shuffle", "Trigger"),
     _in("Reset Seed", "Trigger"),
     _in("In Array", "Float[]"),
     _in("Seed", "Int32", required=False, default=-1),
     _in("Auto Shuffle", "Bool", default=True),
     _in("Enable Shared State", "Bool", default=False)],
    [     _out("On Next", "Trigger"),
...
_register(_node(
    ...
    [     _in("Trigger", "Trigger"),
     _in("Left Array", "Float[]"),
     _in("Right Array", "Float[]")],
    [     _out("Array", "Float[]")],
...
_register(_node(
    "Array Concat (WaveAsset)", "Arrays",
    "Concatenates two WaveAsset arrays into one.",
    [_in("Trigger", "Trigger"),
     _in("Left Array", "WaveAsset[]"),
     _in("Right Array", "WaveAsset[]")],
    [_out("Array", "WaveAsset[]")],
    ["array", "concatenate", "join", "merge", "combine", "waveasset"],
    complexity=1,
    class_name="Array::Concat::WaveAsset:Array",
))

_register(_node(
    "Array Random Get (Int32)", "Arrays",
    "Gets a random element from an Int32 array, optionally weighted.",
    [_in("Next", "Trigger", required=False),
     _in("Reset", "Trigger", required=False),
     _in("In Array", "Int32[]"),
     _in("Weights", "Float[]", required=False),
    ...
    [_out("On Next", "Trigger"),
     ...
     _out("Value", "Int32"),
     _out("Index", "Int32")],
    ["array", "random", "get", "int32", "variation", "selection"],
    complexity=2,
    class_name="Array::Random Get::Int32:Array",
))
...
_register(_node(
    ...
    [     _in("PrimaryOperand", "Float"),
     _in("AdditionalOperands", "Float")],
    [     _out("Out", "Float")],
...
_register(_node(
    ...
    [     _in("PrimaryOperand", "Int32", default=1),
     _in("AdditionalOperands", "Int32", default=1)],
    [     _out("Out", "Int32")],
...
_register(_node(
    ...
    [     _in("In 0", "Audio"),
     _in("Gain 0", "Float", default=1),
     _in("In 1", "Audio", required=False),
     _in("Gain 1", "Float", required=False, default=1)],
    [     _out("Out", "Audio")],
...
_register(_node(
    "Audio Mixer (Mono, 5)", "Mix",
    "5-input mono mixer with per-input gain.",
    [_in("In 0", "Audio"),
     _in("Gain 0", "Float", default=1.0),
     _in("In 1", "Audio", required=False),
     _in("Gain 1", "Float", required=False, default=1.0),
     ...
     _in("In 3", "Audio", required=False),
     _in("Gain 3", "Float", required=False, default=1.0),
     _in("In 4", "Audio", required=False),
     _in("Gain 4", "Float", required=False, default=1.0)],
    [_out("Out", "Audio")],
    ["mix", "mixer", "mono", "sum", "combine", "audio mixer"],
    complexity=1,
    class_name="AudioMixer::Audio Mixer (Mono, 5)::None",
))
...
_register(_node(
    "Audio Mixer (Stereo, 4)", "Mix",
    "Mixes 4 stereo audio inputs with individual gain controls.",
    [_in("In 0 L", "Audio"), _in("In 0 R", "Audio"), _in("Gain 0", "Float", required=False, default=1.0),
     _in("In 1 L", "Audio"), _in("In 1 R", "Audio"), _in("Gain 1", "Float", required=False, default=1.0),
     _in("In 2 L", "Audio"), _in("In 2 R", "Audio"), _in("Gain 2", "Float", required=False, default=1.0),
     _in("In 3 L", "Audio"), _in("In 3 R", "Audio"), _in("Gain 3", "Float", required=False, default=1.0)],
    [_out("Out L", "Audio"), _out("Out R", "Audio")],
    ["mixer", "stereo", "audio", "gain", "summing"],
    complexity=1,
    class_name="AudioMixer::Audio Mixer (Stereo, 4)::None",
))

_register(_node(
    "Audio Mixer (Stereo, 6)", "Mix",
    "Mixes 6 stereo audio inputs with individual gain controls.",
    [_in("In 0 L", "Audio"), _in("In 0 R", "Audio"), _in("Gain 0", "Float", required=False, default=1.0),
     _in("In 1 L", "Audio"), _in("In 1 R", "Audio"), _in("Gain 1", "Float", required=False, default=1.0),
     _in("In 2 L", "Audio"), _in("In 2 R", "Audio"), _in("Gain 2", "Float", required=False, default=1.0),
     _in("In 3 L", "Audio"), _in("In 3 R", "Audio"), _in("Gain 3", "Float", required=False, default=1.0),
     _in("In 4 L", "Audio"), _in("In 4 R", "Audio"), _in("Gain 4", "Float", required=False, default=1.0),
     _in("In 5 L", "Audio"), _in("In 5 R", "Audio"), _in("Gain 5", "Float", required=False, default=1.0)],
    [_out("Out L", "Audio"), _out("Out R", "Audio")],
    ["mixer", "stereo", "audio", "gain", "summing"],
    complexity=1,
    class_name="AudioMixer::Audio Mixer (Stereo, 6)::None",
))

_register(_node(
    "Audio Mixer (Stereo, 7)", "Mix",
    "Mixes 7 stereo audio inputs with individual gain controls.",
    [_in("In 0 L", "Audio"), _in("In 0 R", "Audio"), _in("Gain 0", "Float", required=False, default=1.0),
     _in("In 1 L", "Audio"), _in("In 1 R", "Audio"), _in("Gain 1", "Float", required=False, default=1.0),
     _in("In 2 L", "Audio"), _in("In 2 R", "Audio"), _in("Gain 2", "Float", required=False, default=1.0),
     _in("In 3 L", "Audio"), _in("In 3 R", "Audio"), _in("Gain 3", "Float", required=False, default=1.0),
     _in("In 4 L", "Audio"), _in("In 4 R", "Audio"), _in("Gain 4", "Float", required=False, default=1.0),
     _in("In 5 L", "Audio"), _in("In 5 R", "Audio"), _in("Gain 5", "Float", required=False, default=1.0),
     _in("In 6 L", "Audio"), _in("In 6 R", "Audio"), _in("Gain 6", "Float", required=False, default=1.0)],
    [_out("Out L", "Audio"), _out("Out R", "Audio")],
    ["mixer", "stereo", "audio", "gain", "summing"],
    complexity=1,
    class_name="AudioMixer::Audio Mixer (Stereo, 7)::None",
))

_register(_node(
    "Audio Mixer (Stereo, 8)", "Mix",
    "Mixes 8 stereo audio inputs with individual gain controls.",
    [_in("In 0 L", "Audio"), _in("In 0 R", "Audio"), _in("Gain 0", "Float", required=False, default=1.0),
     _in("In 1 L", "Audio"), _in("In 1 R", "Audio"), _in("Gain 1", "Float", required=False, default=1.0),
     _in("In 2 L", "Audio"), _in("In 2 R", "Audio"), _in("Gain 2", "Float", required=False, default=1.0),
     _in("In 3 L", "Audio"), _in("In 3 R", "Audio"), _in("Gain 3", "Float", required=False, default=1.0),
     ...
     _in("In 5 L", "Audio"), _in("In 5 R", "Audio"), _in("Gain 5", "Float", required=False, default=1.0),
     _in("In 6 L", "Audio"), _in("In 6 R", "Audio"), _in("Gain 6", "Float", required=False, default=1.0),
     _in("In 7 L", "Audio"), _in("In 7 R", "Audio"), _in("Gain 7", "Float", required=False, default=1.0)],
    [_out("Out L", "Audio"), _out("Out R", "Audio")],
    ["mixer", "stereo", "audio", "gain", "summing"],
    complexity=1,
    class_name="AudioMixer::Audio Mixer (Stereo, 8)::None",
))
...
_register(_node(
    ...
    [     _in("In 0 L", "Audio"),
     _in("In 0 R", "Audio", required=False),
     _in("Gain 0", "Float", default=1),
     _in("In 1 L", "Audio", required=False),
     _in("In 1 R", "Audio", required=False),
     _in("Gain 1", "Float", required=False, default=1)],
    [     _out("Out L", "Audio"),
     _out("Out R", "Audio")],
...
_register(_node(
    ...
    [     _in("In", "Audio"),
     _in("Pan Amount", "Float", default=0),
     _in("Panning Law", "Enum", default=0)],
    [     _out("Out Left", "Audio"),
     _out("Out Right", "Audio")],
...
_register(_node(
    ...
    [     _in("In Mid", "Audio"),
     ...
     _in("Equal Power", "Bool", default=False)],
    [     _out("Out Left", "Audio"),
     _out("Out Right", "Audio")],
...
_register(_node(
    ...
    [     _in("Scale Degrees", "Enum", required=False, default=0),
     _in("Chord Tones Only", "Bool", default=False)],
...
_register(_node(
    ...
    [     _in("Next", "Trigger"),
     _in("Reset", "Trigger", required=False),
     _in("Seed", "Int32", required=False, default=-1),
     _in("Min", "Time", default=0),
     _in("Max", "Time", default=1)],
    [     _out("On Next", "Trigger"),
     _out("On Reset", "Trigger"),
     _out("Value", "Time")],
...
_register(_node(
    ...
    [     _in("Audio Bus", "AudioBusAsset", required=False)],
    [     _out("Out 0", "Audio"),
     _out("Out 1", "Audio")],
...
_register(_node(
    ...
    [     _in("Filename Prefix", "String", default="Output"),
...
_register(_node(
    ...
    [     _in("In Audio", "Audio"),
     _in("Modulation Rate", "Float", default=0.5),
     _in("Modulation Depth", "Float", default=0.5),
     _in("Center Delay", "Float", default=0.5),
...
_register(_node(
    ...
    [     _in("In Carrier", "Audio"),
     _in("In Modulator", "Audio")],
    [     _out("Out Audio", "Audio")],
...
_register(_node(
    ...
    [     _in("In", "Audio"),
     _in("Amount", "Float", default=1),
     _in("Bias", "Float", default=0),
     _in("OutputGain", "Float", default=1),
     _in("Type", "Enum", default=0)],
    [     _out("Out", "Audio")],
...
_register(_node(
    "Convert Float to Bool", "General",
    "Converts a Float value to Bool (non-zero = true).",
    [_in("Float", "Float")],
    [_out("Bool", "Bool")],
    ["convert", "type", "cast", "float", "bool"],
    complexity=1,
    class_name="Convert::Float::Bool",
))

_register(_node(
    "Value (Int32)", "General",
    "Holds an Int32 value. Updated on trigger, outputs the stored value.",
    [_in("On Set", "Trigger", required=False),
     _in("Value", "Int32", default=0)],
    [_out("On Set", "Trigger"),
     _out("Value", "Int32")],
    ["value", "constant", "int32", "store", "hold"],
    complexity=1,
    class_name="Value::Value::Int32",
))

_register(_node(
    "Value (Float)", "General",
    "Holds a Float value. Updated on trigger, outputs the stored value.",
    [_in("On Set", "Trigger", required=False),
     _in("Value", "Float", default=0.0)],
    [_out("On Set", "Trigger"),
     _out("Value", "Float")],
    ["value", "constant", "float", "store", "hold"],
    complexity=1,
    class_name="Value::Value::Float",
))

_register(_node(
    "Pipe", "General",
    "Passes audio through unchanged. Useful for explicit signal routing.",
    [_in("In", "Audio")],
    [_out("Out", "Audio")],
    ["pipe", "passthrough", "routing", "utility"],
    complexity=1,
    class_name="UE::Pipe::None",
))
...
_register(_node(
    ...
    [     _in("In", "Audio", required=False),
     _in("Amount", "Float", default=1),
     _in("Bias", "Float", required=False, default=0),
     _in("OutputGain", "Float", required=False, default=1),
     _in("Type", "Enum", default=0)],
    [     _out("Out", "Audio")],
...
_register(_node(
    ...
    [     _in("Bypass", "Bool", required=False, default=False),
     _in("In Left", "Audio"),
     _in("In Right", "Audio"),
     _in("Dry Level", "Float", required=False, default=1),
     _in("Wet Level", "Float", required=False, default=1),
     _in("Delay (ms)", "Float", default=0),
     _in("Gain (dB)", "Float", default=0),
     _in("Bandwidth", "Float", default=0.5),
     _in("Diffusion", "Float", default=0.5),
...
_register(_node(
    ...
    [     _in("Play", "Trigger", required=False),
     _in("Stop", "Trigger", required=False),
     _in("Wave Asset", "WaveAsset"),
     _in("Start Time", "Time", required=False, default=0),
     _in("Pitch Shift", "Float", required=False, default=0),
     _in("Loop", "Bool", required=False, default=False),
     _in("Loop Start", "Time", required=False, default=0),
     _in("Loop Duration", "Time", required=False, default=-1),
     _in("Maintain Audio Sync", "Bool", default=False)],
    [     _out("On Play", "Trigger"),
     _out("On Finished", "Trigger"),
     _out("On Nearly Finished", "Trigger"),
     _out("On Looped", "Trigger"),
...
CLASS_NAME_TO_DISPLAY: dict[str, str] = {
    ...
    "AudioMixer::Audio Mixer (Stereo, 6)::None": "Audio Mixer (Stereo, 6)",
    "AudioMixer::Audio Mixer (Stereo, 7)::None": "Audio Mixer (Stereo, 7)",
    "AudioMixer::Audio Mixer (Stereo, 8)::None": "Audio Mixer (Stereo, 8)",
    "TriggerAny::Trigger Any (3)::None": "Trigger Any (3)",
    "TriggerAny::Trigger Any (4)::None": "Trigger Any (4)",
    "TriggerAccumulator::Trigger Accumulate (3)::None": "Trigger Accumulate (3)",
    "TriggerAccumulator::Trigger Accumulate (4)::None": "Trigger Accumulate (4)",
    "TriggerAccumulator::Trigger Accumulate (5)::None": "Trigger Accumulate (5)",
    "UE::Trigger Sequence (3)::None": "Trigger Sequence (3)",
    "Value::Value::Int32": "Value (Int32)",
    "Value::Value::Float": "Value (Float)",
    "Convert::Float::Bool": "Convert Float to Bool",
