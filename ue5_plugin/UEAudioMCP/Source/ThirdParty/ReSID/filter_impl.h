...
// ----------------------------------------------------------------------------
// SIDKIT v0.1.1: Enable/disable dithering.
// Dithering adds low-level noise to reduce quantization artifacts.
// ----------------------------------------------------------------------------
void Filter::enable_dithering(bool enable)
{
...
// voice: 0-2 (voice index)
// vol: 0-256 where 256 = unity gain (no attenuation), 0 = silent
// Values > 256 allow boost but may cause clipping
// ----------------------------------------------------------------------------
void Filter::set_voice_volume(int voice, int vol)
{
  if (voice >= 0 && voice < 3) {
    voice_vol[voice] = vol;
  }
...
void Filter::set_Q()
{
  ...
  // SIDKIT v0.1.3: Add resonance boost for self-oscillation
  // Stock SID Q: 0.707-1.7 (never self-oscillates)
  // With max boost (255): adds 3.3 to Q, reaching ~5.0 (screaming self-osc)
  double Q_base = 0.707 + 1.0*res/0x0f;
  double Q_boost = (res_boost / 255.0) * 3.3;
  double Q_total = Q_base + Q_boost;

  // The coefficient 1024 is dispensed of later by right-shifting 10 times
  // (2 ^ 10 = 1024).
  _1024_div_Q = static_cast<sound_sample>(1024.0/Q_total);
}

// ----------------------------------------------------------------------------
// SIDKIT v0.1.3: Set resonance boost for self-oscillation.
// boost: 0-255 where 0=stock SID, 255=max self-oscillation (~Q=5.0)
// ----------------------------------------------------------------------------
void Filter::set_resonance_boost(int boost)
{
  res_boost = (boost < 0) ? 0 : (boost > 255) ? 255 : boost;
  set_Q();  // Recalculate Q with new boost value
}
