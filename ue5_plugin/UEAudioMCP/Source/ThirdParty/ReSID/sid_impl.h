...
void SID16::forceDigiOutput( int voice, int value )
{
  forceOutput[ voice ] = value;
}
...
int SID16::output()
{
  ...
  int sample = extfilt.output()/((4095*255 >> 7)*3*15*2/range) + (v0p<<0);
  if (sample >= half) {
    return half - 1;
  }
  if (sample < -half) {
...
// ----------------------------------------------------------------------------
void SID16::readRegisters( unsigned char *p )
{
  p[ 0 ] = voice[2].wave.readOSC();
  p[ 1 ] = voice[2].envelope.readENV();
}
...
// ----------------------------------------------------------------------------
// SIDKIT v0.1.1: Enable dithering.
// ----------------------------------------------------------------------------
void SID16::enable_dithering(bool enable)
{
  filter.enable_dithering(enable);
}

// ----------------------------------------------------------------------------
// SIDKIT v0.1.2: Set per-voice volume.
// ----------------------------------------------------------------------------
void SID16::set_voice_volume(int voice, int vol)
{
  filter.set_voice_volume(voice, vol);
}

// ----------------------------------------------------------------------------
// SIDKIT v0.1.3: Set resonance boost for self-oscillation.
// ----------------------------------------------------------------------------
void SID16::set_resonance_boost(int boost)
{
  filter.set_resonance_boost(boost);
}

// ----------------------------------------------------------------------------
// SIDKIT v0.1.4: Set FM cross-modulation.
// carrier: 0-2 (voice to modulate)
// modulator: 0=off, 1=OSC1, 2=OSC2, 3=OSC3 (matches parameters.yaml)
// depth: 0-255 (modulation intensity)
// ----------------------------------------------------------------------------
void SID16::set_fm_mod(int carrier, int modulator, int depth)
{
  if (carrier >= 0 && carrier < 3) {
    // Schema: 0=off, 1-3=voice -> Internal: -1=off, 0-2=voice
    fm_mod_source[carrier] = (modulator == 0) ? -1 : (modulator - 1);
    fm_mod_depth[carrier] = depth;
  }
}

// ----------------------------------------------------------------------------
// SIDKIT v0.1.7: Set soft sync amount.
// voice: 0-2 (voice to apply soft sync to)
// amount: 0=hard sync (stock), 255=very soft (barely any sync)
// ----------------------------------------------------------------------------
void SID16::set_soft_sync(int voice, int amount)
{
  if (voice >= 0 && voice < 3) {
    this->voice[voice].wave.set_soft_sync(amount);
  }
}
...
/*  if ( ( write_pipeline ) && ( delta_t > 0 ) ) {
  ...
  // SIDKIT v0.1.4: Apply FM phase modulation before waveform generation
  for ( i = 0; i < 3; i++ ) {
      int8_t mod_src = fm_mod_source[i];
      if ( mod_src >= 0 && mod_src < 3 && fm_mod_depth[i] > 0 ) {
          // Use previous waveform output as modulator (1-sample delay)
          // Scale: waveform is 12-bit (0-4095), depth is 0-255
          // Phase offset scaled to accumulator range (24-bit)
          int32_t mod_val = prev_waveform[mod_src] - 2048;  // Center around 0
          int32_t phase_offset = (mod_val * fm_mod_depth[i]) << 4;  // Scale to 24-bit
          voice[i].wave.accumulator = (voice[i].wave.accumulator + phase_offset) & 0xffffff;
      }
  }
  ...
  // Store waveform outputs for next FM cycle
  for ( i = 0; i < 3; i++ ) {
      prev_waveform[i] = voice[i].wave.waveform_output;
  }
