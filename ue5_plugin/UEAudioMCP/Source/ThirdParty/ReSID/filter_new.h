...
#ifndef __FILTER_NEW_H__
#define __FILTER_NEW_H__
...
protected:
  void set_sum_mix();
  void set_w0();
  void set_Q();  // Missing from VICE header - needed by impl
  ...
  // Compatibility members for sid_impl.h state saving
  // voice3off is bit 7 of mode, hp_bp_lp is bits 4-6
  reg8 voice3off;  // Computed from mode in writeMODE_VOL
  reg8 hp_bp_lp;   // Computed from mode in writeMODE_VOL
  ...
  // Missing from VICE header - needed by impl
  int w0;           // Cutoff frequency (for 8580 compatibility path)
  int _8_div_Q;     // Resonance coefficient
  int _1024_div_Q;  // Resonance coefficient (scaled)

  // SIDKIT Extension: Resonance Boost
  bool resBoostEnabled;
  int resBoost;
...
*/
#if 1  // SIDKIT: Enable solve_gain for filter initialization
RESID_INLINE
