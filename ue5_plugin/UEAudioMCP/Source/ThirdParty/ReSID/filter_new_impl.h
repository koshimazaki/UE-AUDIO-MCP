...
//  ---------------------------------------------------------------------------
// Adapted for SIDKIT WASM build

#ifndef __FILTER_NEW_IMPL_H__
#define __FILTER_NEW_IMPL_H__
...
// Emscripten and modern compilers have log1p in math.h
#if defined(__EMSCRIPTEN__) || defined(__clang__) || (defined(__GNUC__) && __GNUC__ >= 4)
#define HAS_LOG1P 1
#endif
...
Filter::Filter()
{
    ...
    if (!class_init) {
        ...
        for (int m = 0; m < 2; m++) {
            ...
            // Fit in 5 bits.
            n_snake = (int)(denorm*(1 << 13)*(fi.uCox/(2*fi.k)*fi.WL_snake*1.0e-6/fi.C) + 0.5);
...
void Filter::reset()
{
    ...
    mode = 0;
    vol = 0;
    voice3off = 0;  // Compatibility
    hp_bp_lp = 0;   // Compatibility
...
void Filter::writeMODE_VOL(reg8 mode_vol)
{
    ...
    // Compatibility: set voice3off and hp_bp_lp for sid_impl.h state saving
